<?php
	...
	function logger($success, $errors, $label) {
	...
	function findMatchingContactId($conn, $ticket) {
	...
	function importEmployeeDepartments($conn) {
	...
	function importEmployeeTitles($conn) {
	...
	function importTickets($conn) {
		...
		if (truncate($conn,$table)) {
			...
			foreach ($tickets as $t) {
				...
				$contact_id = findMatchingContactId($conn,$t);
	...
	function importEquipmentTypes($conn) {
	...
	if (!$conn_ms = mssql_connect(CONVERGENCE_HOST,CONVERGENCE_USER,CONVERGENCE_PASS)) {
		echo "error connecting to ".CONVERGENCE_HOST;
	}

	$conn_my = mysqli_connect(LOCAL_HOST, LOCAL_USER,LOCAL_PASS, LOCAL_DB) or die("error connecting to localhost");

	if (!$link_ms = mssql_query('USE '.CONVERGENCE_DB)) {
		echo "error opening db ".CONVERGENCE_DB;
	}

	mysqli_select_db($conn_my, LOCAL_DB) or die("error opening db ".LOCAL_DB);

	// importCustomers($conn_my);
	// importEmployees($conn_my);
	// importEmployeeDepartments($conn_my);
	// importEmployeeTitles($conn_my);
	importTickets($conn_my);
	// importStatus($conn_my);
	// importPriority($conn_my);
	// importDivision($conn_my);
	// importEquipment($conn_my);
	// importEquipmentTypes($conn_my);
	// importContacts($conn_my);
	// importPosts($conn_my);
	// importUsers($conn_my);
