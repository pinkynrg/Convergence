...
class TicketsController extends BaseController {
    ...
    public static function api($params)
    {
    	$tickets = Ticket::select("tickets.*",
            DB::raw("CASE WHEN tickets.created_at > posts.created_at OR posts.created_at IS NULL THEN tickets.created_at ELSE posts.created_at END as 'last_operation_date'"),
            DB::raw("CASE WHEN tickets.created_at > posts.created_at OR posts.created_at IS NULL THEN tickets.creator_id ELSE posts.author_id END as 'last_operation_company_person_id'")
        );
        ...
        $tickets->leftJoin(DB::raw('(SELECT MAX(id) as post_id, ticket_id FROM posts GROUP BY ticket_id) as d1'),'d1.ticket_id','=','tickets.id');
